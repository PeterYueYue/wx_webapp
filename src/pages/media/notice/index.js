...
const ProductOrder = (props) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        ...
        <>
          <Link to={`/shop/orderDetails?id=${record.orderId}`} >  详情  </Link>
          <Divider type="vertical" />
          {record.state == 3||record.state==1 &&record.roState==-1 ?(
            <span className={styles.option}  onClick={() => {  protectRights(record) }} href="#">维权</span>
          ):null}
          <Divider type="vertical" />
          {record.state == 1?(
           <Popconfirm title="确定发货吗？" onConfirm={() => {editSure(record) } } onCancel={cancel}>
              <span className={styles.option} type="primary">发货</span>
          </Popconfirm>
          ):null}
  ...
  function outExcel(params){
    window.open(api+'sbag-server/back/rest/order/export?'+convertObj(params));
  }
  // Object转Query
  ...
  const editSure= (record) => {//发货
    const arr = [];
    arr.push(record.orderId)
    ...
    <PageHeaderWrapper>
      ...
      <Modal
          ...
          <Form {...formLayout}  form={form}  style={{textAlign:'center'}} >
            <FormItem name="name" label="维权原因"
            rules={[ {required: true, message: '请输入维权原因！', } ]}
            >
